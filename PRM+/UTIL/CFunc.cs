...
        );
        ...
        //버튼의 모서리를 둥글게
        public static void ControlRoundRect(ref Label ctl)
        {
            ctl.Region = Region.FromHrgn(CreateRoundRectRgn(1, 1, ctl.Width, ctl.Height, 5, 5));
        }
        public static void ControlRoundRect(ref Button ctl)
        {
            ctl.Region = Region.FromHrgn(CreateRoundRectRgn(1, 1, ctl.Width, ctl.Height, 5, 5));
        }
        ...
        //무인 계산 번호 생성
        //public static string GenCalKey()
        //{
        //    string sKey = "";
        //    string sDt = CFunc.D2S(DateTime.Now, 8);

        //    if(CData.sLastDt != sDt)
        //    {
        //        CData.nCalCount = 0;
        //        CData.sLastDt = sDt;
        //    }
        ...
        //    CData.nCalCount++;
        //    sKey = "10000" + CData.sLastDt + Zero(CData.nCalCount, 5) ;

        //    CIni.Save("DATA", "CAL_COUNT", CData.nCalCount.ToString(), CData.sIniPath);
        //    CIni.Save("DATA", "LAST_DT", CData.sLastDt, CData.sIniPath);

        //    return sKey;
        //}
        //public static string GenPkKey()
        //{
        //    string sKey = "";

        //    if (CData.nPkCount > 99999)
        //        CData.nPkCount = 0;

        //    CData.nPkCount++;            
        //    sKey = Zero(CData.nCalCount, 5);

        //    CIni.Save("DATA", "PkCount", CData.nPkCount.ToString(), CData.sIniPath);
        ...
        public static int S2B(string sData)
        {
            byte[] koByte = Encoding.Default.GetBytes(sData);

            return koByte.Length;
        }
