...
namespace DH.DB
{
    class CSqlite : CBaseDB
    {
        ...
        public override void CreateDatabase(string strPath)
        {
            try
            {
        ...
        public override bool Open(string sPath)
        {
            try
            {
                ...
                sConquery = sPath;
                //MessageBox.Show(sConquery.ToString());
            }
            catch(Exception ex)
            {
                CLog.LOG(LOG_TYPE.ERR, "ERR : " + ex.ToString());
            }
            finally
            {
            }
        ...
        public override string LoadData_Str(string query)
        {
            if (pConn == null)
                return "";
            ...
            try
            {
                ...
                return pDs.Tables[0].Rows.ToString();
            }
            catch (Exception e)
            {
                CLog.LOG(LOG_TYPE.DATA, "Sqlite::LoadData(" + query + ") - " + e.Message);
        ...
        public override bool AddColumn(string sTable, string sColumn, string sType)
        {
            if (pConn == null)
                return false;
        ...
        public void BuildTable(string sDBName, string[] srTableInfo)
        {
            ...
            for (i = 1; i < srTableInfo.Length; i += 2)
            {
                ...
                if (i + 1 < srTableInfo.Length - 1)
                    squery += sType + sSize + ((i == 1) ? " primary key" : "") + ", ";
                else
                    squery += sType + sSize + " ";
            }
            squery += ");";
            if (!ExecQuery(squery))
            {
                CLog.LOG(LOG_TYPE.ERR, "BuildTable Error: DB-" + sDBName + " SQL: " + squery);
